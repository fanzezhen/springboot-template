...
layui.use(['element', 'form', 'laypage'], function () {
    ...
    Admin.searchPage = function () {
        ajaxCommit(AdminConstant.API.ROLE_LIST_PAGE.URL, CommonConstant.AJAX_TYPE.POST, JSON.stringify(Admin.queryData()), CommonConstant.CONTENT_TYPE.JSON, pageSuccess, alert_error);
    };
    ...
    function pageSuccess(data) {
        ...
        data.records.forEach(function (role) {
            ...
            $tr.append($("<td>" + AdminEnum.ROLE_TYPE[role.roleType] + "</td>"));
            ...
            $aEdit.attr("onclick", "xadmin.open('修改角色', '" + AdminConstant.API.ROLE_EDIT.URL + "?roleId=" + role.id + "', 700, 500)");
...
function delAll() {
    ...
    layer.confirm('确认要删除吗？' + idList.toString(), function () {
        //捉到所有被选中的，发异步进行删除
        ajaxCommit(AdminConstant.API.ROLE_DEL_BATCH.URL, CommonConstant.AJAX_TYPE.POST, "idList=" + idList, CommonConstant.CONTENT_TYPE.FORM, successDel, alert_error);
    });
}
...
function role_stop(roleId, $obj, STATUS) {
    layer.confirm('确认要' + $obj.attr('title') + '吗？', function () {
        ajaxCommit(AdminConstant.API.ROLE_SAVE.URL, CommonConstant.AJAX_TYPE.POST, JSON.stringify({
            id: roleId,
